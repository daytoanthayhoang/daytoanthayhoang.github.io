export function veDe02(containerId, renderLog) {
    ...
    const baseCoords = { A: [0, 4], B: [0, 0], C: [6, 4] };
    ...
    const a1 = board.create('angle', [pC, pB, pD], {radius: 0.8, withLabel: false, visible: false});
    const a2 = board.create('angle', [pD, pB, pA], {radius: 1.0, withLabel: false, visible: false});
    ...
    const angleE = board.create('angle', [pB, pE, pD], {type: 'square', size: 0.3, withLabel: false, strokeColor: '#dc2626', visible: false});
        ...
        nextStep: function() {
            if (step === 1) {
                ...
                <ul class="list-none space-y-2 pl-2 text-sm border-l-2 border-slate-600 ml-1">
                   <li>- Xét hai tam giác vuông $ABD$ và $EBD$ có: <span class="float-right text-amber-400 font-bold">0.25đ</span></li>
                   <li>- $BD$ là cạnh huyền chung <span class="float-right text-amber-400 font-bold">0.25đ</span></li>
                   <li>- $\\widehat{ABD} = \\widehat{EBD}$ (do $BD$ là phân giác) <span class="float-right text-amber-400 font-bold">0.25đ</span></li>
                   <li class="font-bold text-emerald-400 pt-1">- Vậy $\\Delta ABD = \\Delta EBD$ (ch-gn) <span class="float-right text-amber-400 font-bold">0.25đ</span></li>
                </ul>`;
                renderLog(baremA);
                step++;
            } 
                ...
                <ul class="list-none space-y-2 pl-2 text-sm border-l-2 border-slate-600 ml-1">
                   <li>- Xét $\\Delta ADM$ và $\\Delta EDC$ có: $AD = ED$ (cmt), $\\widehat{ADM} = \\widehat{EDC}$ (đối đỉnh) <br>$\\Rightarrow \\Delta ADM = \\Delta EDC$ (cgv-gnk) <span class="float-right text-amber-400 font-bold">0.5đ</span></li>
                   <li>- Suy ra $AM = EC$ (hai cạnh tương ứng). Lại có $BA = BE$ (cmt) <span class="float-right text-amber-400 font-bold">0.25đ</span></li>
